...
class ProductQuotationEmail extends Mailable
{
    ...
    {
        $templateData = EmailTemplate::query()->where('type', EmailTemplate::PRODUCT_QUOTATION)->where('enabled', 1)->first();
        if (empty($templateData)){
            info('产品报价-发送邮件通知', ['error' => '未找到对应的邮件模板或者邮件模板未启用']);
