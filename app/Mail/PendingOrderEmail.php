...
class PendingOrderEmail extends Mailable
{
    ...
    {
        $templateData = EmailTemplate::query()->where('type', EmailTemplate::PENDING_PAYMENT_ORDER)->where('enabled', 1)->first();
        if (empty($templateData)){
            info('待支付订单-发送邮件通知', ['error' => '未找到对应的邮件模板或者邮件模板未启用']);
