...
/**
 * 客户线下充值记录导出
 * Class OfflineRechargeRecordExport
 * @package App\Jobs\Export
 * @author DonnyLiu <2365057581@qq.com>
 * @date 2024/12/08 16:11
 */
class OfflineRechargeRecordExport implements ShouldQueue
{
    ...
    {
        $storagePath = '/excel/offline_recharge/';
        ...
        try {
            ...
            $urlFile = $excel->fileName($fileName, '客户线下充值记录')
                ...
                if (is_array($images) && !empty($images[0])) {
                    ...
                    foreach ($images as $index => $imageUrl) {
                        if ($imageUrl) {
                            ...
                            // 插入链接
                            $urlFile->insertUrl($picRow, 9, $imageUrl, NULL, '流水号[' . $row[0] . ']的第' . (1 + $index) . '张核账图', $urlStyle);
                        }
                    }
                }
            }
            ...
            info('客户线下充值记录导出失败:' . $throwable);

            return;
        }

        info('客户线下充值记录导出成功: '. $path);
