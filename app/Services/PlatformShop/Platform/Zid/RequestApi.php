...
class RequestApi
{
    ...
    public function __construct($shop)
    {
        ...
        if (!empty($shop->id)) {
            ...
            $this->authorizationToken = $shop->ext_data['authorization_token'] ?? '';
        }
     ...
     * 获取授权token
     * @param $authCode
     * @param string $grantType 授权类型 authorization_code 授权登录，refresh_token 刷新token
     * @return mixed
     * @throws GuzzleException
     * @throws Exception
     */
    public function authorize($authCode, string $grantType = 'authorization_code')
    {
        $url = '/oauth/token';
        $option = [
        ...
        $this->authorizationToken = $result['data']['Authorization'];
    ...
    protected function request(string $uri, string $method = 'GET', array $query = [], array $data = []): mixed
    {
        ...
        try {
            ...
            info('zid 接口返回', [$option, $content]);
            return $content;
        } catch (GuzzleException $e) {
            info('zid 接口请求失败', [$option, $e->getMessage()]);
            throw new AccidentException('zid 接口请求失败', Code::OPERATE_FAIL);
        }
    }
    ...
    public function setConfig()
    {
        ...
        if (empty($config)) {
            throw new AccidentException("请先设置ZID平台配置", Code::OPERATE_FAIL);
        }
