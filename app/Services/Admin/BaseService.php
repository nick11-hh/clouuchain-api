...
class BaseService
{
        ...
        foreach ($this->filterRules as $k => $v) {
            if (Arr::has($this->formData, $v[1])) {
                ...
                if (is_array($v[1])) {
                    $this->filters[$k][1] = [];
                    foreach ($v[1] as $v1) {
                        if($this->formData[$v1] != '0' && empty($this->formData[$v1])) {
                            unset($this->filters[$k]);
                            break;
                        }
                        $this->filters[$k][1][] = $this->formData[$v1];
                    }
                } else {
                    if($this->formData[$v[1]] != '0' && empty($this->formData[$v[1]])) {
                        unset($this->filters[$k]);
                    } else {
    ...
    // {
    //     $this->query->withoutGlobalScope(CompanyScope::class);
    //
    //     return $this;
    // }
    ...
    public function pageSize(int $perPage = 10)
    {
        if(isset($this->formData['size'])) {
            $perPage = $this->formData['size'];
     ...
     */
    public function getList(array $where = [], array $selectFields = ['*'], array $groupFields = [], array $orderFields = [])
    {
        if (!empty($where)) {
    ...
    {
        return collect($list)->map(function ($value, $key) {
            return [
                'id' => $key,
                'name' => $value,
            ];
