...
class JiaYouQService extends Logistics
{
    protected string $url;

    protected const ACTION_FORECAST_ORDER = '/api/WayBill/CreateOrder'; //运单申请
    protected const ACTION_OBTAIN_CHANNEL_CODE = '/api/channel/getChannel'; //获取渠道代码
    ...
    protected string $channel = 'jiayou_q';
    ...
    public function getLabel(string $sn, LogisticsApplyModel $logisticsApply)
    {
        try {
            ...
            info('佳邮 获取面单失败', ['exception' => $ex->getMessage()]);
            return false;
        }
    ...
    {
        Log::channel('logistics')->info('yuntu-申请物流单号-1');
        ...
        Log::channel('logistics')->info('yuntu-申请物流单号-申报信息-3', [$applyData]);
        ...
        Log::channel('logistics')->info('yuntu-申请物流单号-申报结果-4', [$res]);
    ...
    {
        Log::channel('logistics')->info('yuntu-request', [
            'data' => $data,
        ...
        try {
            ...
            Log::channel('logistics')->info('yuntu-订单佳邮对接失败', [
        ...
        Log::channel('logistics')->info('yuntu-request-response', [$res]);
    ...
    public function tracking(string $sn)
    {
        ...
        try {
            ...
            info('订单佳邮对接失败', ['msg' => $exception->getMessage()]);
    ...
    protected function loadConfig()
    {
        $config = CompanyDockingInfoModel::query()
            ->where('type', OrderDockingRecordModel::TYPE_JIA_YOU_QUAN)
            ->first();
        ...
        if ($config) {
            ...
            throw new AccidentException('尚未配置 佳邮 配置信息', Code::OPERATE_FAIL);
        }
    }
