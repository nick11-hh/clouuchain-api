...
namespace App\Services\ExpressCompanies\YunSuDi;
 ...
 * 云速递
 */
class YunSuDiService extends Logistics
{
    ...
    protected string $channel = 'yunsudi';
    ...
    public function place($package, $logisticsApply)
    {
        ...
        Log::channel('logistics')->info('yunsudi-申请物流单号-订单状态-1', [$package->status]);
        ...
        Log::channel('logistics')->info('yunsudi-申请物流单号-申报信息-3', $data);

        $res = $this->post(self::ACTION_FORECAST_ORDER, $data);

        Log::channel('logistics')->info('yunsudi-申请物流单号-申报结果-4', [$res]);

        if (empty($res) || $res['success'] != 1) {  //申报失败
    ...
    protected function request(string $method, array $data)
    {
        ...
        try {
            ...
            Log::channel('logistics')->info('云速递网络请求超时，请重新对接', [
                'msg' => $exception->getMessage()
            ]);

            return ['msg' => $exception->getMessage()];
        } catch (GuzzleException $exception) {
            Log::channel('logistics')->info('云速递创建订单对接失败', [
                'msg' => $exception->getMessage()
            ]);
    ...
    protected function loadConfig()
    {
        $config = CompanyDockingInfoModel::query()
            ->where('type', OrderDockingRecordModel::TYPE_YUNSUDI)
            ->first();
        ...
        if ($config) {
            ...
            Log::channel('logistics')->info('公司尚未配置云速递对接信息，对接失败');
            throw new AccidentException('尚未配置 云速递 配置信息', Code::OPERATE_FAIL);
        }
    }
