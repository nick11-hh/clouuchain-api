...
class CouponInfo extends JsonResource
{
    public function toArray($request)
    {
        return [
                ...
                'expired_count' => $this->userCoupons->filter(function ($item) {
                    return now()->gt($item->expired_at);
                })->count(),
                'invalid_count' =>  $this->invalid_count,
                'share_qr_code' => $this->share_qr_code ?? '',
                'share_status' => $this->shareStatus,
                'code' => CouponCodeList::collection($this->codes),
